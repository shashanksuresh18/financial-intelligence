import type { EvidenceClass, PeerComparisonItem, WithheldSection } from "@/lib/types";
            ...
            {items.length === 0 ? (
              <tr>
                <td className="px-4 py-4 text-zinc-500" colSpan={6}>
                  {withheld
                    ? `${withheld.userMessage} — fewer than 3 peers passed business-model relevance screening.`
                    : "No peer-comparison data is available yet. Add an FMP API key to unlock this section for supported public companies."}
                </td>
              </tr>
            ) : (
              items.map((item) => (
                <tr key={`${item.symbol}-${item.companyName}`}>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <EvidenceClassBadge
                        evidenceClass={item.evidenceClass ?? "market-data-vendor"}
                      />
                    </div>
