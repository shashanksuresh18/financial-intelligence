...
function SectionCard({
  ...
  return (
    <section
      className={`rounded-3xl border border-zinc-800 bg-zinc-950/70 p-4 ${className}`}
    >
        ...
        <div className="mt-3 space-y-3">
          {items.map((item) => (
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                <span
                  className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] ${STRESS_TEST_SEVERITY_STYLES[item.severity]}`}
                >
                  {item.severity}
                </span>
              </div>
...
export function InvestmentMemoPanel({ memo }: InvestmentMemoPanelProps) {
  return (
    <section className="rounded-3xl border border-zinc-800 bg-gradient-to-br from-zinc-950 via-zinc-950 to-emerald-950/10 p-5 shadow-[0_24px_80px_-48px_rgba(0,0,0,0.95)]">
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {memo.keyRisks.length === 0 ? (
            ...
            memo.keyRisks.map((risk) => (
              <RiskCard key={`${risk.rank}-${risk.title}`} risk={risk} />
            ))
          )}
        </div>
      </SectionCard>
      ...
      {memo.stressTest ? (
        <SectionCard className="mt-4" title="Stress Test">
          <div className="space-y-4">
            {memo.stressTest.convictionDowngraded ? (
              <div className="rounded-2xl border border-rose-400/20 bg-rose-400/10 p-4">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full border border-rose-400/20 bg-rose-400/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-rose-200">
                    Conviction Downgraded
                  </span>
                  <span className="text-xs uppercase tracking-[0.18em] text-rose-100/85">
                    Original conviction{' '}
                    {CONVICTION_LABELS[memo.stressTest.originalConviction]}
                  </span>
                </div>
              </div>
            ) : null}
